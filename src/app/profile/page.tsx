...
export default function ProfilePage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (form.password) {
      changes.passwordHash = await hashPassword(form.password);
    }
    ...
    setSuccess('회원정보가 수정되었습니다. 다음 로그인부터 수정한 휴대폰번호를 사용합니다.');
  }
    ...
    <>
      <section className="subHero">
        <div className="siteShell subHeroInner">
          ...
          <h1>회원 정보수정</h1>
          <p>연락처 등 기본정보를 수정합니다. 소속 클럽은 이적·신규·탈퇴 처리 결과에 따라 자동 변경됩니다.</p>
        </div>
      </section>
      ...
      <div className="siteShell pageContent">
        <form className={styles.wrap} onSubmit={handleSubmit}>
          ...
          <div className={styles.row}>
            <label>직책</label>
            <input value={user.position || '회원'} readOnly className={styles.readOnly} />
          </div>
